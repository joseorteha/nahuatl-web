...
export default function Recompensas({ userId }: { userId: string }) {
  ...
  const obtenerRecompensas = useCallback(async () => {
    try {
      ...
      const contribucionesResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/contribuciones/stats/${userId}`, {
        headers: {
      ...
      const historialResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/contributions/historial/${userId}`, {
        headers: {
          'Authorization': `Bearer ${parsedTokens?.accessToken}`,
          'Content-Type': 'application/json'
        },
